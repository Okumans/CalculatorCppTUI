...
template <typename T, typename E = std::exception> class [[nodiscard]] Result {
private:
...
public:
  ...
  Result(Result &&other) noexcept
      : mValueOrException(std::move(other.mValueOrException)) {}

  bool isError() const { return std::holds_alternative<E>(mValueOrException); }

  const T &getValue() const { return std::get<T>(mValueOrException); }

  const E &getException() const { return std::get<E>(mValueOrException); }
  ...
  T moveValue() { return std::get<T>(std::move(mValueOrException)); }
};
