...
inline bool NodePointer::isTypeValid(const std::unordered_map<std::string, Lambda>& EvaluatorLambdaFunction) const {
	if (Result<RuntimeType, std::runtime_error> targetType{ getExpressionReturnType(mNodeExpression, EvaluatorLambdaFunction, &NodeFactory::getNodesCachedType()) }; targetType.isError())
		return false;
	return true;
}
