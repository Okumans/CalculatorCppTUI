...
inline RuntimeCompoundType::RuntimeCompoundType(
    const RuntimeCompoundType &other)
    : mHashed{other.mHashed}, Type{other.Type}, Children{other.Children} {
...
inline RuntimeCompoundType::RuntimeCompoundType(
    RuntimeCompoundType &&other) noexcept
    : mHashed{other.mHashed}, Type{std::move(other.Type)},
      Children{std::move(other.Children)} {}

// Copy assignment operator for RuntimeCompoundType
inline RuntimeCompoundType &
...
// Create a RuntimeCompoundType with a vector of RuntimeType elements
inline RuntimeCompoundType
RuntimeCompoundType::Storage(const std::vector<RuntimeType> &base) {
  for (const RuntimeType &element : base)
    assert((std::holds_alternative<RuntimeBaseType>(element) &&
            static_cast<int8_t>(std::get<RuntimeBaseType>(element)) <= 3) ||
           (std::holds_alternative<RuntimeCompoundType>(element) &&
            static_cast<int8_t>(std::get<RuntimeCompoundType>(element).Type) <=
                3));
...
// Create a RuntimeCompoundType with a rvalue vector of RuntimeType elements
inline RuntimeCompoundType
RuntimeCompoundType::Storage(std::vector<RuntimeType> &&base) {
  for (const RuntimeType &element : base)
    assert(
        (std::holds_alternative<RuntimeBaseType>(element) &&
         static_cast<int8_t>(std::get<RuntimeBaseType>(element)) < 3) ||
        (std::holds_alternative<RuntimeCompoundType>(element) &&
         static_cast<int8_t>(std::get<RuntimeCompoundType>(element).Type) < 3));

  return RuntimeCompoundType(RuntimeBaseType::_Storage, std::move(base));
}

// Create a RuntimeCompoundType representing a lambda with return type and
// parameters
inline RuntimeCompoundType
RuntimeCompoundType::Lambda(const RuntimeType &Ret, const RuntimeType &Params) {
  return RuntimeCompoundType(RuntimeBaseType::_Lambda, {Ret, Params});
}

// Create a RuntimeCompoundType representing a lambda with return type and
// parameters
inline RuntimeCompoundType RuntimeCompoundType::Lambda(RuntimeType &&Ret,
                                                       RuntimeType &&Params) {
  std::vector<RuntimeType> tmp;
  tmp.reserve(2);
...
inline RuntimeCompoundType
RuntimeCompoundType::RuntimeEvaluateLambdaInfix(const RuntimeType &Ret) {
  return RuntimeCompoundType(RuntimeBaseType::_Operator_Lambda_Infix, {Ret});
}

inline RuntimeCompoundType
RuntimeCompoundType::RuntimeEvaluateLambdaInfix(RuntimeType &&Ret) {
  std::vector<RuntimeType> tmp;
  tmp.emplace_back(std::move(Ret));
  return RuntimeCompoundType(RuntimeBaseType::_Operator_Lambda_Infix,
                             std::move(tmp));
}

inline RuntimeCompoundType
RuntimeCompoundType::RuntimeEvaluateLambdaPostfix(const RuntimeType &Ret) {
  return RuntimeCompoundType(RuntimeBaseType::_Operator_Lambda_Postfix, {Ret});
}

inline RuntimeCompoundType
RuntimeCompoundType::RuntimeEvaluateLambdaPostfix(RuntimeType &&Ret) {
  std::vector<RuntimeType> tmp;
  tmp.emplace_back(std::move(Ret));
  return RuntimeCompoundType(RuntimeBaseType::_Operator_Lambda_Postfix,
                             std::move(tmp));
}

inline RuntimeCompoundType
RuntimeCompoundType::RuntimeEvaluateLambdaPrefix(const RuntimeType &Ret) {
  return RuntimeCompoundType(RuntimeBaseType::_Operator_Lambda_Prefix, {Ret});
}

inline RuntimeCompoundType
RuntimeCompoundType::RuntimeEvaluateLambdaPrefix(RuntimeType &&Ret) {
  std::vector<RuntimeType> tmp;
  tmp.emplace_back(std::move(Ret));
  return RuntimeCompoundType(RuntimeBaseType::_Operator_Lambda_Prefix,
                             std::move(tmp));
}

inline RuntimeCompoundType
RuntimeCompoundType::RuntimeEvaluateLambdaConstant(const RuntimeType &Ret) {
  return RuntimeCompoundType(RuntimeBaseType::_Operator_Lambda_Prefix, {Ret});
}

inline RuntimeCompoundType
RuntimeCompoundType::RuntimeEvaluateLambdaConstant(RuntimeType &&Ret) {
  std::vector<RuntimeType> tmp;
  tmp.emplace_back(std::move(Ret));
  return RuntimeCompoundType(RuntimeBaseType::_Operator_Lambda_Constant,
                             std::move(tmp));
}

// Parse a string representing a RuntimeType
inline Result<RuntimeType, std::runtime_error>
RuntimeCompoundType::ParseString(const std::string &stringLikeType) {
  ...
  for (const std::string &lexeme : lexemes) {
    ...
    if (lexeme == "]") {
      ...
      while (!operationStack.empty() &&
             !std::holds_alternative<char>(operationStack.top())) {
        containRuntimeType.push_back(
            std::get<RuntimeType>(operationStack.top()));
        operationStack.pop();
      }
      ...
      if (std::holds_alternative<char>(operationStack.top()) ||
          std::holds_alternative<RuntimeCompoundType>(
              std::get<RuntimeType>(operationStack.top())) ||
          std::get<RuntimeBaseType>(std::get<RuntimeType>(
              operationStack.top())) == RuntimeBaseType::Number)
      ...
      // RuntimeCompoundType
      RuntimeBaseType runtimeTypeElementHolder = std::get<RuntimeBaseType>(
          std::get<RuntimeType>(operationStack.top()));
      operationStack.pop();
      ...
      if (runtimeTypeElementHolder == RuntimeBaseType::_Storage)
        ...
        operationStack.emplace(RuntimeCompoundType::Lambda(
            containRuntimeType[0], containRuntimeType[1]));
      else
        return RuntimeError<RuntimeTypeError>(
...
inline RuntimeCompoundType::StorageInfo
RuntimeCompoundType::getStorageInfo(const RuntimeType &storage) {
  assert(std::get_if<RuntimeCompoundType>(&storage)->Type ==
         RuntimeBaseType::_Storage);

  const RuntimeCompoundType &storageCompound =
      std::get<RuntimeCompoundType>(storage);
...
inline RuntimeCompoundType::LambdaInfo
RuntimeCompoundType::getLambdaInfo(const RuntimeType &lambda) {
  assert(std::get_if<RuntimeCompoundType>(&lambda)->Type ==
         RuntimeBaseType::_Lambda);

  const RuntimeCompoundType &lambdaCompound =
      std::get<RuntimeCompoundType>(lambda);

  if (std::holds_alternative<RuntimeCompoundType>(lambdaCompound.Children[1]))
    return LambdaInfo{std::make_shared<RuntimeType>(lambdaCompound.Children[0]),
                      std::make_shared<RuntimeType>(lambdaCompound.Children[1]),
                      getStorageInfo(lambdaCompound.Children[1]).StorageSize};

  if (std::get<RuntimeBaseType>(lambdaCompound.Children[1]) ==
      RuntimeBaseType::_Storage)
    return LambdaInfo{std::make_shared<RuntimeType>(lambdaCompound.Children[0]),
                      std::make_shared<RuntimeType>(lambdaCompound.Children[1]),
                      0};
...
inline RuntimeType
RuntimeCompoundType::_getLambdaParamsType(const RuntimeType &lambda) {
  assert(std::get_if<RuntimeCompoundType>(&lambda)->Type ==
         RuntimeBaseType::_Lambda);
...
inline RuntimeType
RuntimeCompoundType::_getLambdaReturnType(const RuntimeType &lambda) {
  assert(std::get_if<RuntimeCompoundType>(&lambda)->Type ==
         RuntimeBaseType::_Lambda);
...
inline size_t
RuntimeCompoundType::_getLambdaParamsNumbers(const RuntimeType &lambda) {
  assert(std::get_if<RuntimeCompoundType>(&lambda)->Type ==
         RuntimeBaseType::_Lambda);
  ...
  if (std::holds_alternative<RuntimeCompoundType>(lambdaCompound.Children[1])) {
    assert(
        std::get_if<RuntimeCompoundType>(&lambdaCompound.Children[1])->Type ==
        RuntimeBaseType::_Storage);
    return std::get<RuntimeCompoundType>(lambdaCompound.Children[1])
        .Children.size();
  }
...
inline size_t RuntimeCompoundType::_getHash() const { return mHashed; }

// Equality operator for RuntimeType, use hash to compare types.
inline bool operator==(const RuntimeType &runtimeType1,
                       const RuntimeType &runtimeType2) {
  return std::hash<RuntimeType>{}(runtimeType1) ==
         std::hash<RuntimeType>{}(runtimeType2);
}
...
inline std::ostream &operator<<(std::ostream &os, const RuntimeType &et) {
  if (const RuntimeBaseType *
          runtimeBaseType{std::get_if<RuntimeBaseType>(&et)};
      runtimeBaseType)
    os << *runtimeBaseType;

  else if (const RuntimeCompoundType *
               runtimeCompoundType{std::get_if<RuntimeCompoundType>(&et)};
           runtimeCompoundType) {
...
inline std::string RuntimeTypeToString(const RuntimeType &rt) {
  ...
  if (const RuntimeBaseType *
          runtimeBaseType{std::get_if<RuntimeBaseType>(&rt)};
      runtimeBaseType)
    oss << *runtimeBaseType;

  else if (const RuntimeCompoundType *
               runtimeCompoundType{std::get_if<RuntimeCompoundType>(&rt)};
           runtimeCompoundType) {
    switch (runtimeCompoundType->Type) {
    case RuntimeBaseType::Number:
      oss << "ERROR_Number";
...
inline size_t
RuntimeCompoundType::generateHash(RuntimeBaseType wrapper,
                                  const std::vector<RuntimeType> &base) const {
  size_t seed = 0;
  ...
  for (const auto &child : base) {
    if (std::holds_alternative<RuntimeCompoundType>(child))
      ...
      hash_combine(seed, std::hash<char>{}(static_cast<char>(
                             std::get<RuntimeBaseType>(child))));
  }
  return seed;
}

inline size_t RuntimeCompoundType::generateHash(RuntimeBaseType wrapper,
                                                const RuntimeType &base) const {
  size_t seed = 0;
  ...
  if (std::holds_alternative<RuntimeCompoundType>(base))
    ...
    hash_combine(seed, std::hash<char>{}(
                           static_cast<char>(std::get<RuntimeBaseType>(base))));
...
inline std::size_t
std::hash<RuntimeType>::operator()(const RuntimeType &rt) const {
...
template <> struct std::formatter<RuntimeType> : std::formatter<std::string> {
