...
constexpr std::array color_codes = {
	...
	"\033[94m", "\033[95m", "\033[96m", "\033[97m", "\033[0m" };
...
template <Color color> constexpr std::string ColorText(const char* text) {
...
constexpr std::string ColorText(const std::string& text) {
	return color_codes[static_cast<std::size_t>(color)] + text +
		color_codes[static_cast<std::size_t>(Color::Reset)];
}
...
inline std::string HighlightSyntax(const std::string& code) {
	...
	// highlighted_code = boost::regex_replace(highlighted_code,
	// bracket_contents_regex, ColorText<Color::Bright_Yellow>("$&"));

	// Highlight numbers
	highlighted_code = boost::regex_replace(highlighted_code, number_regex,
		ColorText<Color::Bright_Blue>("$&"));

	// Highlight types
	highlighted_code = boost::regex_replace(highlighted_code, type_regex,
		ColorText<Color::Green>("$&"));

	// Highlight types
	highlighted_code = boost::regex_replace(highlighted_code, operators_regex,
		ColorText<Color::Cyan>("$&"));

	// Highlight brackets
	highlighted_code = boost::regex_replace(highlighted_code, bracket_regex,
